...
def main():
    ...
    if args.eval_path is not None:
        ...
        print('The test model is loaded')
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0:
            save_file = os.path.join(args.save_path, str(epoch) + 'epoch' + '.pth')
            save_model(model, args.epochs, save_file)
            print('evaluating ' + str(epoch) + 'epoch' + '.pth')
            evaluate_fewshot(model.encoder, test_loader, n_way=args.n_way, n_shots=[1, 5], n_query=args.n_query,
