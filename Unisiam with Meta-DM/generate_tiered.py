...
ROOT_PATH = './data/tiered-imagenet/'
prompt = "natural"

def random_pick(setname):
    df = pd.read_csv(osp.join('split','tieredImageNet' ,setname + '.csv'))
...
def generate_bad(setname):
    ...
    for l in lines:
        ...
        filepath = osp.join(ROOT_PATH, 'ext')
...
def update_splits(setname):
    ...
    output_file = osp.join('split', 'tieredImageNet', setname + '.csv')
